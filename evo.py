...
class Evo:
    def __init__(self, net_no=500, input_no=1,output_no=1,
                 ...
                 empty=False, custom_net_class=None,
    ...
    def next_gen(self):
        ...
        for ni1 in range(self.elite_no, self.chosen_no):
            if self.mode == "reproduction":
                ...
                new_net = self.nets[ni1].reproduce_with(self.nets[ni2], class_to_use=self.net_class)
            elif self.mode == "mutation":
                new_net = self.nets[ni1].mutate(mutation_rate=self.mutation_rate)
    ...
    def fill_with_nets(self):
        while len(self.nets) < self.net_no:
            weights = generator.network(input_no=self.input_no,output_no=self.output_no,min_n=self.input_no,max_n=self.max_neurons_per_net)
            self.nets.append(self.net_class(weights, input_no=self.input_no,output_no=self.output_no))
